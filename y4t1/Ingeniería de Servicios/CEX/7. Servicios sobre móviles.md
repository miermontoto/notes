...
[[_resources/Tema 7. Servicios sobre móviles.pdf]]
...
- El hilo del GUI **se detiene** si la actividad es *pausada* o *detenida*.
	- **Es destruido y creado de nuevo ante un cambio de configuración (ej. cambio de orientación)**
...
> [!faq] Bastante tiempo dedicado a esta parte.

### Bundling
Típicamente hay dos tipos de transferencias de datos:
- **Datos dependientes del tiempo:** iniciadas por el usuario a través de la interfaz → se espera respuesta inmediata.
- **Datos independientes del tiempo:** iniciados por la aplicación.

> [!note] La idea es retrasar y acumular las independientes y realizarlas cuando se inicie una dependiente del tiempo.

**Implementación**
- Los métodos que implementen transferencias *inependientes* guardarán en una cola los datos relativos a la misma.
- Los métodos que implementan transferencias *dependientes* incorporarán llamadas a métodso que transmiten también la cola de pendientes.

**Soluciones**
- Android: `ContentProvider` manual o bien `SyncAdapter` (ya implementado por Google)

> [!faq] Poco tiempo dedicado a esta parte


### Notificaciones (eliminación del polling)
Para eliminar el *polling*, es necesario que permita una conexión TCP permanente entre el dispositivo y el servidor, y por la cual el servidor pueda enviar datos nuevos cuando estos estén presentes.

*Alternativas: implementar protocolo propio, usar XMPP, usar tecnologías propietarias (FCM, APN)*

![[_resources/Pasted image 20231129184908.png]]
> [!faq] Explicó entero este diagrama